...
const Predictor = () => {
  ...
  // Create dynamic title and description based on the available prediction data
  const predictionTitle = predictionData && predictionData.length > 0 
    ? `${predictionData[0]?.first_name || ''} ${predictionData[0]?.last_name || ''}'s Prediction | LuckyOrGenius`
    : "Prediction Details | LuckyOrGenius";

  const predictionDescription = predictionData && predictionData.length > 0 
    ? `${predictionData[0]?.prediction?.slice(0, 150)}${predictionData[0]?.prediction?.length > 150 ? '...' : ''}`
    : "View detailed analysis of this prediction and its accuracy on LuckyOrGenius";

  const predictionKeywords = predictionData && predictionData.length > 0
    ? `prediction, ${predictionData[0]?.category || ''}, forecasting, accuracy, ${predictionData[0]?.first_name || ''} ${predictionData[0]?.last_name || ''}`
    : "prediction, forecasting, accuracy, influencer";
